...
- **3D 臺灣**：Cesium World Terrain 地形（高度放大 2.5 倍），縣市依圖層著色，可切換「即時氣溫／預報最高／預報最低／降雨機率」與預報日期
...
3. **Vercel**：匯入 GitHub repository（Framework 選 Other），在 Environment Variables 設定上表四組變數後部署。`vercel.json` 已經設定部署區域 `hnd1`、路由與每日排程。
