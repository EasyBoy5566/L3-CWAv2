...
- **建築模型與陰影模擬**：Google 實景 3D（Photorealistic 3D Tiles，經 Cesium ion），依太陽高度調整清晨、黃昏、夜晚的光線，並依最近測站的天氣變灰或起霧；按播放鍵就在目前的地點從早上開始播放一天的日照，建築陰影隨太陽移動（鏡頭太高時先降到畫面中央）。北／中／南按鍵飛到臺北 101、中興大學、高雄 85 大樓。拉近時地形自動回到真實比例，讓建築貼合地面
